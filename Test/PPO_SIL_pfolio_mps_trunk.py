...
class MarketConfig:
    ...
    initial_holding_noise: float = 5.0
...
class PPOSILConfig:
    ...
    # Shared-trunk SIL coefficients.  The older decision/impulse-specific
    # coefficients below are kept for checkpoint/notebook compatibility.
    ...
    log_every: int = 1
    checkpoint_path: str = "PPO_SIL_pfolio_mps_trunk.pt"
...
class MLPTrunk(nn.Module):
    """Shared MLP feature extractor used by policy and critic heads."""
    ...
    def __init__(self, obs_dim: int, hidden_dims: Tuple[int, ...], activation: str, layer_norm: bool) -> None:
        ...
        self.out_dim = int(last)

    def forward(self, obs: torch.Tensor) -> torch.Tensor:
        return self.net(obs)


class SharedTrunkPortfolioAgent(nn.Module):
    """Shared-trunk PPO-SIL portfolio agent.

    The trunk is shared by
        * the Bernoulli intervention head,
        * the diagonal-Gaussian trade-intent head, and
        * the scalar critic value head.

    The Gaussian is defined on an unconstrained raw action.  The executed trade
    intent is tanh-squashed and scaled by ``max_trade_size``.  The environment
    subsequently projects the intent to cash, min_holding, max_holding and
    max_trade_size constraints.
    """
    ...
    def __init__(self, obs_dim: int, action_dim: int, cfg: PPOSILConfig, max_trade_size: torch.Tensor) -> None:
        ...
        self.intervene_head = nn.Linear(self.trunk.out_dim, 1)
        self.trade_mean_head = nn.Linear(self.trunk.out_dim, self.action_dim)
        ...
        orthogonal_init_(self.trade_mean_head, gain=0.01)
        orthogonal_init_(self.value_head, gain=1.0)
        nn.init.constant_(self.intervene_head.bias, float(cfg.decision_bias))
    ...
    def dist_and_value(
        self,
        obs: torch.Tensor,
    ) -> Tuple[torch.Tensor, torch.distributions.Normal, torch.Tensor, torch.Tensor]:
        h = self.features(obs)
        intervene_logits = self.intervene_head(h).squeeze(-1)
        raw_mean = self.trade_mean_head(h)
        log_std = self.log_std.clamp(float(self.cfg.min_log_std), float(self.cfg.max_log_std))
        raw_std = torch.exp(log_std).expand_as(raw_mean)
        raw_dist = torch.distributions.Normal(raw_mean, raw_std)
        value = self.value_head(h).squeeze(-1)
        return intervene_logits, raw_dist, raw_mean, value

    def raw_to_trade(self, raw_action: torch.Tensor) -> torch.Tensor:
        scale = self.max_trade_size.to(raw_action.device, raw_action.dtype)
        return torch.tanh(raw_action) * scale
    ...
    def evaluate_actions(
        ...
        intervene_logits, raw_dist, _raw_mean, value = self.dist_and_value(obs)
        bernoulli = torch.distributions.Bernoulli(logits=intervene_logits)
        intervene_f = intervene.to(value.dtype).view(-1)
        raw_action = raw_action.view(-1, self.action_dim)

        logp_d = bernoulli.log_prob(intervene_f)
        entropy_d = bernoulli.entropy()
        prob_d = torch.sigmoid(intervene_logits)

        logp_u = raw_dist.log_prob(raw_action).sum(dim=-1)
        entropy_u = raw_dist.entropy().sum(dim=-1)

        joint_logp = logp_d + intervene_f * logp_u
        joint_entropy = entropy_d + prob_d.detach() * entropy_u
        return {
            "logp_d": logp_d,
            "entropy_d": entropy_d,
            "prob_d": prob_d,
    ...
    def act(self, obs: torch.Tensor, deterministic: bool = False) -> Dict[str, torch.Tensor]:
        intervene_logits, raw_dist, raw_mean, value = self.dist_and_value(obs)
        bernoulli = torch.distributions.Bernoulli(logits=intervene_logits)
        prob_d = torch.sigmoid(intervene_logits)
        ...
        if deterministic:
            ...
            raw_action = raw_dist.rsample()

        trade_intent = self.raw_to_trade(raw_action)
        ev = self.evaluate_actions(obs, intervene, raw_action)
        return {
            "intervene": intervene,
            ...
            "logp_d": ev["logp_d"],
            "logp_u": ev["logp_u"],
            "joint_logp": ev["joint_logp"],
            "entropy_d": ev["entropy_d"],
            "entropy_u": ev["entropy_u"],
            "joint_entropy": ev["joint_entropy"],
            "value": value,
            "prob_d": prob_d,
        }


# Backward-compatible class name used by some notebooks.
PPOPortfolioAgent = SharedTrunkPortfolioAgent
...
@torch.no_grad()
def rollout_batch(
    agent: SharedTrunkPortfolioAgent,
    ...
    state = env.reset(n_envs)
    initial_wealth = env.wealth(state)
    initial_discounted_wealth = env.discounted_wealth(state)
    ...
    old_logp_buf = torch.empty(steps, n_envs, device=env.device, dtype=env.dtype)
    old_logp_d_buf = torch.empty(steps, n_envs, device=env.device, dtype=env.dtype)
    old_logp_u_buf = torch.empty(steps, n_envs, device=env.device, dtype=env.dtype)
    value_buf = torch.empty(steps, n_envs, device=env.device, dtype=env.dtype)
    ...
    for t in range(steps):
        ...
        old_logp_buf[t] = act["joint_logp"]
        old_logp_d_buf[t] = act["logp_d"]
        old_logp_u_buf[t] = act["logp_u"]
    ...
    terminal_profit = terminal_wealth - initial_wealth
    discounted_profit = discounted_terminal_wealth - initial_discounted_wealth
    return {
        "obs": obs_buf,
        ...
        "old_logp_d": old_logp_d_buf,
        "old_logp_u": old_logp_u_buf,
        "value": value_buf,
        "reward": reward_buf,
        "reward_actual": reward_actual_buf,
        "initial_wealth": initial_wealth,
        "initial_discounted_wealth": initial_discounted_wealth,
        ...
        "terminal_return": terminal_profit / initial_wealth.clamp_min(1.0e-12),
        "discounted_return": discounted_profit / initial_discounted_wealth.clamp_min(1.0e-12),
...
def compute_advantages(rollout: Dict[str, torch.Tensor], cfg: PPOSILConfig) -> None:
    returns, adv_raw, adv = _gae_and_returns(rollout["reward"], rollout["value"], cfg)
    rollout["mc_return"] = returns
    rollout["return"] = returns
    rollout["advantage"] = adv
    rollout["advantage_raw"] = adv_raw


def flatten_rollout(rollout: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    skip = {
        "initial_wealth", "initial_discounted_wealth", "terminal_wealth",
        "discounted_terminal_wealth", "terminal_profit", "discounted_profit",
        "terminal_return", "discounted_return", "trade_count", "total_cost",
        "mean_abs_trade", "final_h", "final_s", "final_cash",
    }
...
class PrioritizedSILBuffer:
    def __init__(
        ...
        self.value = torch.empty(self.capacity, device=self.storage_device, dtype=dtype)
    ...
    def add_batch(
        ...
        self.value[idx] = value.detach().to(self.storage_device, non_blocking=False)
    ...
    def sample(self, batch_size: int) -> Tuple[Dict[str, torch.Tensor], torch.Tensor]:
        ...
        batch = {
            ...
            "value": self.value[idx].to(self.device, non_blocking=False),
        }
        return batch, idx
...
def add_to_sil(buffer: PrioritizedSILBuffer, rollout: Dict[str, torch.Tensor], cfg: PPOSILConfig) -> None:
    ...
    gap = (flat["mc_return"] - flat["value"] - float(cfg.sil_margin)).clamp_min(0.0)
    priority = gap + float(cfg.sil_priority_eps)
    buffer.add_batch(flat["obs"], flat["intervene"], flat["raw_action"], flat["mc_return"], flat["value"], priority)
...
def _safe_sharpe_np(x: np.ndarray, eps: float = 1.0e-12) -> float:
    ...
    if sd <= eps:
        return 0.0 if abs(mu) <= eps else float(np.sign(mu) * np.inf)
    return mu / sd


def _max_drawdown_np(values: np.ndarray) -> float:
    arr = np.asarray(values, dtype=float).reshape(-1)
    if arr.size == 0:
        return float("nan")
    running_max = np.maximum.accumulate(arr)
    dd = arr / np.maximum(running_max, 1.0e-12) - 1.0
    return float(np.min(dd))


def _ppo_minibatch_loss(agent: SharedTrunkPortfolioAgent, batch: Dict[str, torch.Tensor], cfg: PPOSILConfig) -> Tuple[torch.Tensor, Dict[str, float]]:
    ev = agent.evaluate_actions(batch["obs"], batch["intervene"], batch["raw_action"])

    ratio = torch.exp(ev["joint_logp"] - batch["old_logp"])
    surr1 = ratio * batch["advantage"]
    surr2 = ratio.clamp(1.0 - float(cfg.clip_eps), 1.0 + float(cfg.clip_eps)) * batch["advantage"]
    policy_loss = -torch.min(surr1, surr2).mean()
    value_loss = 0.5 * (ev["value"] - batch["return"]).pow(2).mean()
    entropy = ev["joint_entropy"].mean()

    loss = policy_loss + float(cfg.value_coef) * value_loss - float(cfg.entropy_coef) * entropy
    kl = (batch["old_logp"] - ev["joint_logp"]).mean().abs()
    kl_d = (batch["old_logp_d"] - ev["logp_d"]).mean().abs()
    mask = batch["intervene"].to(torch.bool)
    if mask.any():
        kl_u = (batch["old_logp_u"][mask] - ev["logp_u"][mask]).mean().abs()
        impulse_entropy = ev["entropy_u"][mask].mean()
    else:
        kl_u = torch.zeros((), device=batch["obs"].device, dtype=batch["obs"].dtype)
        impulse_entropy = torch.zeros((), device=batch["obs"].device, dtype=batch["obs"].dtype)

    stats = {
        "policy_loss": float(policy_loss.detach().cpu()),
        "value_loss": float(value_loss.detach().cpu()),
        "entropy": float(entropy.detach().cpu()),
        "decision_entropy": float(ev["entropy_d"].mean().detach().cpu()),
        "impulse_entropy": float(impulse_entropy.detach().cpu()),
        "kl": float(kl.detach().cpu()),
        "kl_decision": float(kl_d.detach().cpu()),
        "kl_impulse": float(kl_u.detach().cpu()),
    }
    return loss, stats


def _sil_coef(cfg: PPOSILConfig, name: str, fallback_name: str) -> float:
    return float(getattr(cfg, name, getattr(cfg, fallback_name)))


def _sil_update(agent: SharedTrunkPortfolioAgent, opt: torch.optim.Optimizer, sil: PrioritizedSILBuffer, cfg: PPOSILConfig) -> Dict[str, float]:
    batch, idx = sil.sample(int(cfg.sil_batch_size))
    ev = agent.evaluate_actions(batch["obs"], batch["intervene"], batch["raw_action"])
    gap = (batch["mc_return"] - ev["value"] - float(cfg.sil_margin)).clamp_min(0.0)
    pos = gap > 0.0
    zero = torch.zeros((), device=batch["obs"].device, dtype=batch["obs"].dtype)
    if pos.any():
        policy_loss = -(gap[pos].detach() * ev["joint_logp"][pos]).sum() / gap[pos].detach().sum().clamp_min(1.0)
        value_loss = 0.5 * gap[pos].pow(2).mean()
    else:
        policy_loss = zero
        value_loss = zero

    loss = _sil_coef(cfg, "sil_policy_coef", "sil_decision_policy_coef") * policy_loss + _sil_coef(cfg, "sil_value_coef", "sil_decision_value_coef") * value_loss
    ...
    priority = gap.detach() + float(cfg.sil_priority_eps)
    sil.update_priority(idx, priority)
    return {
        "sil_policy_loss": float(policy_loss.detach().cpu()),
        "sil_value_loss": float(value_loss.detach().cpu()),
    }
...
def evaluate_policy(
    ...
    """Evaluate a trained policy over many independent SSG paths.

    Sharpe ratios are computed from terminal simple returns across the Monte
    Carlo batch.  Discounted Sharpe ratios use discounted terminal returns.
    Annualized versions divide by sqrt(T), where T is measured in years.
    """
    n_envs = int(n_envs)
    batch_size = int(batch_size or n_envs)

    initial, terminal, disc_terminal, profit, disc_profit = [], [], [], [], []
    returns, disc_returns, trades, costs, abs_trades = [], [], [], [], []
    h_sum = None
    s_sum = None
    cash_sum = None
    left = n_envs
    while left > 0:
        ...
        initial.append(rollout["initial_wealth"].detach().cpu())
        terminal.append(rollout["terminal_wealth"].detach().cpu())
        disc_terminal.append(rollout["discounted_terminal_wealth"].detach().cpu())
        profit.append(rollout["terminal_profit"].detach().cpu())
        disc_profit.append(rollout["discounted_profit"].detach().cpu())
        returns.append(rollout["terminal_return"].detach().cpu())
        disc_returns.append(rollout["discounted_return"].detach().cpu())
    ...
    initial_np = torch.cat(initial).numpy()
    terminal_np = torch.cat(terminal).numpy()
    disc_terminal_np = torch.cat(disc_terminal).numpy()
    profit_np = torch.cat(profit).numpy()
    disc_profit_np = torch.cat(disc_profit).numpy()
    returns_np = torch.cat(returns).numpy()
    disc_returns_np = torch.cat(disc_returns).numpy()
    trades_np = torch.cat(trades).numpy()
    costs_np = torch.cat(costs).numpy()
    abs_trade_np = torch.cat(abs_trades).numpy()

    sharpe = _safe_sharpe_np(returns_np)
    disc_sharpe = _safe_sharpe_np(disc_returns_np)
    annual_factor = 1.0 / math.sqrt(max(float(env.cfg.T), 1.0e-12))

    return {
        "initial_wealth_mean": float(np.mean(initial_np)),
        "terminal_wealth_mean": float(np.mean(terminal_np)),
        "terminal_wealth_std": float(np.std(terminal_np, ddof=0)),
        "discounted_terminal_wealth_mean": float(np.mean(disc_terminal_np)),
        "discounted_terminal_wealth_std": float(np.std(disc_terminal_np, ddof=0)),
        "terminal_profit_mean": float(np.mean(profit_np)),
        "terminal_profit_std": float(np.std(profit_np, ddof=0)),
        "discounted_profit_mean": float(np.mean(disc_profit_np)),
        "discounted_profit_std": float(np.std(disc_profit_np, ddof=0)),
        "terminal_return_mean": float(np.mean(returns_np)),
        "terminal_return_std": float(np.std(returns_np, ddof=0)),
        "discounted_return_mean": float(np.mean(disc_returns_np)),
        "discounted_return_std": float(np.std(disc_returns_np, ddof=0)),
        "sharpe_ratio": float(sharpe),
        "annualized_sharpe_ratio": float(sharpe * annual_factor),
        "discounted_sharpe_ratio": float(disc_sharpe),
        "annualized_discounted_sharpe_ratio": float(disc_sharpe * annual_factor),
        "trade_count_mean": float(np.mean(trades_np)),
        "trade_count_std": float(np.std(trades_np, ddof=0)),
        "total_cost_mean": float(np.mean(costs_np)),
        "total_cost_std": float(np.std(costs_np, ddof=0)),
        "mean_abs_trade": float(np.mean(abs_trade_np)),
        ...
        "terminal_wealth_paths": terminal_np,
        "discounted_terminal_wealth_paths": disc_terminal_np,
        "terminal_return_paths": returns_np,
        "discounted_return_paths": disc_returns_np,
        "trade_count_paths": trades_np,
        "total_cost_paths": costs_np,
    }
...
def evaluate_single_path(
    ...
    """Run one SSG path and return a full trace plus scalar path metrics."""
    ...
    cash: List[float] = []
    wealth: List[float] = []
    discounted_wealth: List[float] = []

    trade_times: List[float] = []
    interventions: List[int] = []
    trade_intents: List[np.ndarray] = []
    safe_trades: List[np.ndarray] = []
    costs: List[float] = []
    rewards_actual: List[float] = []
    probs_intervene: List[float] = []

    def record_state(st: Dict[str, torch.Tensor]) -> None:
        t_val = float(env.time(st)[0].detach().cpu())
        time_values.append(t_val)
        prices.append(st["s"][0].detach().cpu().numpy().copy())
        holdings.append(st["h"][0].detach().cpu().numpy().copy())
        cash.append(float(st["cash"][0].detach().cpu()))
        wealth.append(float(env.wealth(st)[0].detach().cpu()))
        discounted_wealth.append(float(env.discounted_wealth(st)[0].detach().cpu()))

    record_state(state)
    for _ in range(steps):
        current_time = float(env.time(state)[0].detach().cpu())
        obs = env.observe(state)
        act = agent.act(obs, deterministic=deterministic)
        next_state, reward, _done, info = env.step(state, act["intervene"], act["trade_intent"])

        trade_times.append(current_time)
        interventions.append(int(act["intervene"][0].detach().cpu()))
        trade_intents.append(act["trade_intent"][0].detach().cpu().numpy().copy())
        safe_trades.append(info["safe_trade"][0].detach().cpu().numpy().copy())
        costs.append(float(info["cost"][0].detach().cpu()))
        rewards_actual.append(float(info["reward_actual"][0].detach().cpu()))
        probs_intervene.append(float(act["prob_d"][0].detach().cpu()))

        state = next_state
        record_state(state)

    price_arr = np.asarray(prices, dtype=float)
    holding_arr = np.asarray(holdings, dtype=float)
    cash_arr = np.asarray(cash, dtype=float)
    wealth_arr = np.asarray(wealth, dtype=float)
    disc_wealth_arr = np.asarray(discounted_wealth, dtype=float)
    risky_values = holding_arr * price_arr
    risky_total = risky_values.sum(axis=1)
    risky_weights = risky_values / np.maximum(wealth_arr[:, None], 1.0e-12)
    risky_allocation = risky_values / np.maximum(risky_total[:, None], 1.0e-12)
    cash_weight = cash_arr / np.maximum(wealth_arr, 1.0e-12)

    trade_arr = np.asarray(safe_trades, dtype=float)
    trade_intent_arr = np.asarray(trade_intents, dtype=float)
    trade_indicator = (np.abs(trade_arr).sum(axis=1) > 1.0e-8).astype(float)

    step_returns = np.diff(wealth_arr) / np.maximum(wealth_arr[:-1], 1.0e-12)
    disc_step_returns = np.diff(disc_wealth_arr) / np.maximum(disc_wealth_arr[:-1], 1.0e-12)
    path_sharpe = _safe_sharpe_np(step_returns)
    disc_path_sharpe = _safe_sharpe_np(disc_step_returns)
    annual_step_factor = math.sqrt(float(env.cfg.n_steps) / max(float(env.cfg.T), 1.0e-12))

    metrics = {
        "initial_wealth": float(wealth_arr[0]),
        "terminal_wealth": float(wealth_arr[-1]),
        "terminal_profit": float(wealth_arr[-1] - wealth_arr[0]),
        "terminal_return": float(wealth_arr[-1] / max(wealth_arr[0], 1.0e-12) - 1.0),
        "discounted_terminal_wealth": float(disc_wealth_arr[-1]),
        "discounted_profit": float(disc_wealth_arr[-1] - disc_wealth_arr[0]),
        "discounted_return": float(disc_wealth_arr[-1] / max(disc_wealth_arr[0], 1.0e-12) - 1.0),
        "sharpe_ratio": float(path_sharpe),
        "annualized_sharpe_ratio": float(path_sharpe * annual_step_factor),
        "discounted_sharpe_ratio": float(disc_path_sharpe),
        "annualized_discounted_sharpe_ratio": float(disc_path_sharpe * annual_step_factor),
        "max_drawdown": _max_drawdown_np(wealth_arr),
        "discounted_max_drawdown": _max_drawdown_np(disc_wealth_arr),
        "trade_count": float(trade_indicator.sum()),
        "total_cost": float(np.sum(costs)),
        "mean_abs_trade": float(np.mean(np.abs(trade_arr))) if trade_arr.size else 0.0,
        "mean_intervention_probability": float(np.mean(probs_intervene)) if probs_intervene else 0.0,
    }
    trace = {
        "time": np.asarray(time_values, dtype=float),
        "prices": price_arr,
        "holdings": holding_arr,
        "cash": cash_arr,
        "wealth": wealth_arr,
        "portfolio_value": wealth_arr,
        "discounted_wealth": disc_wealth_arr,
        "discounted_portfolio_value": disc_wealth_arr,
        "risky_values": risky_values,
        "risky_weights": risky_weights,
        "risky_allocation": risky_allocation,
        "cash_weight": cash_weight,
        "trade_time": np.asarray(trade_times, dtype=float),
        "intervene": np.asarray(interventions, dtype=int),
        "trade_intent": trade_intent_arr,
        "safe_trade": trade_arr,
        "cost": np.asarray(costs, dtype=float),
        "reward_actual": np.asarray(rewards_actual, dtype=float),
        "prob_intervene": np.asarray(probs_intervene, dtype=float),
        "step_returns": step_returns,
        "discounted_step_returns": disc_step_returns,
    }
...
def evaluate_and_plot_single_path(
    agent: SharedTrunkPortfolioAgent,
    env: VarianceGammaPortfolioEnv,
    deterministic: bool = True,
    asset_names: Optional[List[str]] = None,
    include_cash_in_allocation: bool = False,
    save_dir: Optional[str | Path] = None,
    show: bool = True,
) -> Dict[str, Any]:
    """Evaluate one path and produce allocation, impulse, and value plots."""
    result = evaluate_single_path(agent, env, deterministic=deterministic)
    trace = result["trace"]
    ...
    if asset_names is None:
        asset_names = [str(i) for i in range(d)]
    else:
        asset_names = [str(x) for x in asset_names]
        if len(asset_names) != d:
            raise ValueError(f"asset_names must have length d={d}; got {len(asset_names)}.")
    ...
    if include_cash_in_allocation:
        allocation = np.column_stack([trace["cash_weight"], trace["risky_weights"]])
        labels = ["Cash"] + asset_names
        title = "PPO-SIL allocation"
    else:
        allocation = trace["risky_allocation"]
        labels = asset_names
        title = "PPO-SIL risky allocation"

    fig1, ax1 = plt.subplots(figsize=(12, 6))
    ax1.stackplot(t, allocation.T, labels=labels)
    ax1.set_title(title)
    ax1.set_xlabel("Time")
    ...
    if save_path is not None:
        fig1.savefig(save_path / "ppo_sil_risky_allocation.png", dpi=160, bbox_inches="tight")
    figures["allocation"] = fig1

    fig2, ax2 = plt.subplots(figsize=(12, 5))
    trade_time = trace["trade_time"]
    safe_trade = trace["safe_trade"]
    for j, name in enumerate(asset_names):
        mask = np.abs(safe_trade[:, j]) > 1.0e-8
        if np.any(mask):
            ax2.scatter(trade_time[mask], safe_trade[mask, j], s=18, label=name)
        else:
            ax2.scatter([], [], s=18, label=name)
    ax2.axhline(0.0, linewidth=1.0)
    ax2.set_title("PPO-SIL impulses through time")
    ax2.set_xlabel("Time")
    ax2.set_ylabel("Executed impulse / trade in shares")
    ax2.legend(loc="center left", bbox_to_anchor=(1.02, 0.5), fontsize=8)
    fig2.tight_layout()
    if save_path is not None:
        fig2.savefig(save_path / "ppo_sil_impulses_through_time.png", dpi=160, bbox_inches="tight")
    figures["impulses"] = fig2

    fig3, ax3 = plt.subplots(figsize=(12, 5))
    ax3.plot(t, trace["portfolio_value"], linewidth=1.5, label="Portfolio value")
    ax3.plot(t, trace["discounted_portfolio_value"], linewidth=1.5, label="Discounted portfolio value")
    ax3.set_title("PPO-SIL portfolio value")
    ax3.set_xlabel("Time")
    ax3.set_ylabel("Value")
    ax3.legend()
    fig3.tight_layout()
    if save_path is not None:
        fig3.savefig(save_path / "ppo_sil_portfolio_value.png", dpi=160, bbox_inches="tight")
    figures["portfolio_value"] = fig3
    ...
    return {"trace": trace, "metrics": metrics, "figures": figures}
...
def train_ppo_sil(market: Optional[MarketConfig] = None, cfg: Optional[PPOSILConfig] = None) -> Dict[str, Any]:
    ...
    agent = SharedTrunkPortfolioAgent(env.obs_dim, env.action_dim, cfg, env.max_trade_size).to(device=device, dtype=dtype)
    ...
    history: Dict[str, List[Any]] = {k: [] for k in [
        "iter", "policy_loss", "value_loss", "entropy", "decision_entropy", "impulse_entropy",
        "kl", "kl_decision", "kl_impulse", "sil_policy_loss", "sil_value_loss",
        "train_terminal_wealth_mean", "train_discounted_terminal_wealth_mean",
        "train_terminal_profit_mean", "train_discounted_profit_mean", "train_terminal_return_mean",
        "train_discounted_return_mean", "train_trade_count_mean", "train_total_cost_mean",
        "train_intervention_rate", "train_mean_abs_trade", "eval_terminal_wealth_mean",
        "eval_discounted_terminal_wealth_mean", "eval_terminal_profit_mean",
        "eval_discounted_profit_mean", "eval_terminal_return_mean", "eval_discounted_return_mean",
        "eval_sharpe_ratio", "eval_annualized_sharpe_ratio", "eval_discounted_sharpe_ratio",
        "eval_annualized_discounted_sharpe_ratio", "eval_trade_count_mean", "eval_total_cost_mean",
        "eval_mean_abs_trade",
    ]}
    ...
    for it in range(1, int(cfg.train_iters) + 1):
        ...
        acc = {k: 0.0 for k in ["policy_loss", "value_loss", "entropy", "decision_entropy", "impulse_entropy", "kl", "kl_decision", "kl_impulse"]}
        ppo_steps = 0
        stop_early = False
        agent.train()
        for _epoch in range(int(cfg.ppo_epochs)):
            ...
            for start in range(0, total, int(cfg.minibatch_size)):
                ...
                batch = {k: v[idx] for k, v in flat.items() if k in {
                    "obs", "intervene", "raw_action", "old_logp", "old_logp_d", "old_logp_u", "advantage", "return"
                }}
                ...
                for k in acc:
                    acc[k] += stats[k]
                ppo_steps += 1
                if stats["kl"] > float(cfg.target_kl):
                    stop_early = True
                    break
            if stop_early:
                break

        sil_acc = {k: 0.0 for k in ["sil_policy_loss", "sil_value_loss"]}
        sil_steps = 0
        ...
        eval_stats = {k: float("nan") for k in [
            "terminal_wealth_mean", "discounted_terminal_wealth_mean", "terminal_profit_mean",
            "discounted_profit_mean", "terminal_return_mean", "discounted_return_mean",
            "sharpe_ratio", "annualized_sharpe_ratio", "discounted_sharpe_ratio",
            "annualized_discounted_sharpe_ratio", "trade_count_mean", "total_cost_mean", "mean_abs_trade"
        ]}
        if do_eval:
        ...
        history["train_terminal_profit_mean"].append(float(rollout["terminal_profit"].mean().detach().cpu()))
        history["train_discounted_profit_mean"].append(float(rollout["discounted_profit"].mean().detach().cpu()))
        history["train_terminal_return_mean"].append(float(rollout["terminal_return"].mean().detach().cpu()))
        history["train_discounted_return_mean"].append(float(rollout["discounted_return"].mean().detach().cpu()))
        ...
        history["eval_terminal_profit_mean"].append(eval_stats["terminal_profit_mean"])
        history["eval_discounted_profit_mean"].append(eval_stats["discounted_profit_mean"])
        history["eval_terminal_return_mean"].append(eval_stats["terminal_return_mean"])
        history["eval_discounted_return_mean"].append(eval_stats["discounted_return_mean"])
        history["eval_sharpe_ratio"].append(eval_stats["sharpe_ratio"])
        history["eval_annualized_sharpe_ratio"].append(eval_stats["annualized_sharpe_ratio"])
        history["eval_discounted_sharpe_ratio"].append(eval_stats["discounted_sharpe_ratio"])
        history["eval_annualized_discounted_sharpe_ratio"].append(eval_stats["annualized_discounted_sharpe_ratio"])
        ...
        if it == 1 or it % max(1, int(cfg.log_every)) == 0 or it == int(cfg.train_iters):
            print(
                f"[PPO-SIL-trunk iter {it:04d}/{cfg.train_iters}] "
                f"pi={history['policy_loss'][-1]:.3e} v={history['value_loss'][-1]:.3e} "
                f"ent={history['entropy'][-1]:.3f} kl={history['kl'][-1]:.2e} "
                ...
                f"eval_SR={history['eval_annualized_discounted_sharpe_ratio'][-1]:.3f}"
            )
        if cfg.clear_cache:
...
def save_ppo_sil(agent: SharedTrunkPortfolioAgent, env: VarianceGammaPortfolioEnv, market: MarketConfig, cfg: PPOSILConfig, history: Dict[str, Any], path: str | Path) -> None:
    payload = {
        ...
        "history": history,
        "architecture": "shared_trunk_portfolio",
    }
...
def load_ppo_sil(path: str | Path, device: Optional[str] = None) -> Dict[str, Any]:
    ...
    agent = SharedTrunkPortfolioAgent(env.obs_dim, env.action_dim, cfg, env.max_trade_size).to(device=dev, dtype=dtype)
