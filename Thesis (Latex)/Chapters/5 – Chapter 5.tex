...
Having shown in the previous chapter that the value function is a viscosity solution of \eqref{eqn: QVI}, we now prove that it is the unique viscosity solution by means of the \textit{comparison principle}. As the uniqueness argument requires detailed analysis of the integral term in the infinitesimal generator, a few additional assumptions are stated in Section \ref{section: Preliminary Results for the Proof of Uniqueness}. Then, in Sections \ref{section: Equivalent Definitions of Viscosity Solutions} and \ref{section: Maximum Principle}, the original definition of the viscosity solution (see Definition \ref{def: Definition 4.1 in Seydel (Viscosity solution)}) is expressed in terms of functionals and the maximum principle for the parabolic QVI is stated. Finally, the comparison principle and the uniqueness result are proven in Section \ref{section: Proof of the Uniqueness}. 

As in Chapter \ref{section: Viscosity Solution for Impulse Control: Existence}, this chapter closely follows the work of \textcite[Section 5]{seydel2010generalexistenceuniquenessviscosity}. In addition, we also use \cite{barles2008second, jakobsen2006maximum}. It should be noted that although many of the results provided in this chapter are taken directly from \cite{seydel2010generalexistenceuniquenessviscosity}, some proofs did not consider the parabolic case and were therefore not as detailed as the ones we provide.

\section{Preliminary results and further assumptions}
\label{section: Preliminary Results for the Proof of Uniqueness}
Throughout this chapter (and this chapter only), $v$ no longer denotes the value function, $\alpha$ no longer denotes the impulse control, and some other variables may serve new purposes as well. With this in mind, recall the infinitesimal generator \eqref{eqn: infinitesimal generator (short form)} and the parabolic QVI \eqref{eqn: QVI}. As we study the infinitesimal generator $\InfinitesimalGenerator$ more closely in this chapter, we introduce, for $0<\delta<1$, $t \in [0,T)$, $x,p \in \R^d$, $X \in \R^{d \times d}$, and $r,l \in \R$, the following functionals:
\begin{align}
    F(t,x,p,X,l) &= - \frac{1}{2} \Tr\{ \sigma (t,x) \sigma^\top (t,x) X \} - \langle \mu (t,x), p \rangle - f(t,x) - l \label{eqn: functional F} \\
    \mathcal{I}^{1,\delta}[t,x,\varphi] &= \int\limits_{|z|<\delta} \big[ \varphi(t,x+\gamma(t,x,z)) - \varphi(t,x) - \langle \nabla_x \varphi (t,x), \gamma(t,x,z) \rangle \big] \nu(\diff z) \label{eqn: functional I1} \\
    \mathcal{I}^{2,\delta}[t,x,p,\varphi] &= \int\limits_{|z|\geq \delta} \big[ \varphi(t,x+\gamma(t,x,z)) - \varphi(t,x) - \langle p, \gamma(t,x,z) \rangle \ind{|z|<1} \big] \nu(\diff z) \label{eqn: functional I2} \\
    \mathcal{I}[t,x,\varphi] &= \mathcal I^{1,\delta}[t,x,\varphi] + \mathcal  I^{2,\delta}[t,x,\nabla_x \varphi(t,x),\varphi]. \label{eqn: functional I} 
\end{align}
...
\end{align}
where the notation $u(t, \cdot)$ in the integral means that nonlocal terms different from $x$ are used on $u$. Note that the functional $F$ does not directly depend on the zeroth order derivative (i.e. the function $u$ itself) in our case. However, results provided in this chapter are still applicable to cases when the zeroth order derivative is present in the functional $F$, see the elliptical case in \cite[Section 5]{seydel2010generalexistenceuniquenessviscosity}. Properties of the functionals $F$ and $\mathcal{I}$ can be found below.
...
Let us briefly discuss conditions \assitemref{ass: B1} and \assitemref{ass: B2}. \assitemref{ass: B1} imposes an ``integrated'' Lipschitz condition on the jump amplitude $\gamma$, which ensures that the nonlocal Lévy integral varies continuously when comparing it to nearby points. \assitemref{ass: B2} is the corresponding local Lipschitz condition for the diffusion, drift, and running payoff functions seen in Chapter \ref{section: Preliminaries}. Together, these assumptions provide regularity conditions for the application of the \textit{doubling-of-variables} argument in the proof of the comparison principle, both for the local differential terms and for the nonlocal jump term in the infinitesimal generator $\InfinitesimalGenerator$.

We now consider the following result, which is needed in the proof of the maximum principle in Chapter \ref{section: Maximum Principle}. 

\begin{proposition}[{\cite[Proposition 5.1]{seydel2010generalexistenceuniquenessviscosity}}]
\label{prop: Proposition 5.1 in Seydel}
    Recall the functionals \eqref{eqn: functional F}--\eqref{eqn: functional I}. Let $\{t_k\}_{k \geq 0} \subset [0,T)$ and $\{x_k\}_{k \geq 0}, \{p_k\}_{k \geq 0} \subset \R^d$, with $t_k \rightarrow t \in [0,T)$, $x_k \rightarrow x \in \R^d$, $p_k \rightarrow p$. 
...
\begin{proof}
    ...
    \item This is a direct result from (i), (ii), and \assitemref{ass: P1}. \hfill $\qedhere$
    \end{enumerate}
\end{proof}

\section{Equivalent definitions of viscosity solutions}
\label{section: Equivalent Definitions of Viscosity Solutions}
In this section, we restate the definitions of a viscosity solution using the functionals \eqref{eqn: functional I}--\eqref{eqn: functional I}.
...
We now introduce another definition of the viscosity solution. We motivate this by noting that the Lévy measure $\nu$ is typically singular near the origin, so we split $\mathcal I$ into a small-jump part $\mathcal I^{1,\delta}$, evaluated on a smooth test function $\varphi$ at points $(t,x) \in [0,T] \times U_\delta(t,x)$, and a large-jump part $\mathcal I^{2,\delta}$, which evaluates $u$ and preserves monotonicity and semincontinuity under limits. 
...
\label{def: Limiting parabolic semijets}
    Recall Definition \ref{def: Parabolic semijets}, where parabolic super- and subjets are formalised. Let $u: [0,T] \times \R^d \rightarrow \R$ be an USC function and $v:[0,T] \times \R^d \rightarrow \R$ be an LSC function. We define the set of limiting superjets of $u$ at $(t,x) \in [0,T] \times \R^d$ with
    \begin{align*}
...
We now introduce a concept that plays a central role in the proof of the comparison principle, namely the \emph{maximum principle}. Following \textcite{barles2008second} and \textcite{seydel2010generalexistenceuniquenessviscosity}, we begin by providing the parabolic, nonlocal Jensen-Ishii Lemma \ref{lemma: Parabolic, nonlocal Jensen-Ishii Lemma}. 
...
Before considering the maximum principle, we justify the use of the nonlocal Jensen-Ishii lemma in our nonlocal setting instead of the local version. In the local PDE case, the doubling-of-variables argument only requires limiting semijets for the sub- and supersolution. Since the operator is local, the continuity of the operator is sufficient to pass to the limit in the corresponding viscosity inequalities. 

However, in the case of PIDEs, this argument requires an additional approximation step. Recall that the infinitesimal generator $\InfinitesimalGenerator$ contains the nonlocal integral operator $\mathcal I$, and that the small jump part of the integral operator is evaluated on a smooth test function. Since the Lévy measure may be singular at the origin (for example, the Variance-Gamma measure in Example \ref{ex: VG measure} is singular at mark $z=0$), limiting semijets alone cannot provide sufficient control on the integral term. Therefore, following \textcite{barles2008second}, we use the nonlocal version of the Jensen-Ishii Lemma. In particular, for fixed $\alpha>0$, Lemma \ref{lemma: Parabolic, nonlocal Jensen-Ishii Lemma} provides a sequence of smooth test functions $\{\varphi_k\}_{k\geq0}$ that converge over compact sets in $C^{1,2}$ to the ``modified sup convolution'' $\varphi_\alpha$. This will be important for the analysis of the singular small jump integral.

Finally, we obtain the parabolic maximum principle for impulse control, which is a direct application of the nonlocal Jensen-Ishii Lemma \ref{lemma: Parabolic, nonlocal Jensen-Ishii Lemma}. The following result is taken from \textcite{barles2008second} and \textcite{seydel2010generalexistenceuniquenessviscosity}, while \textcite{jakobsen2006maximum} provide a similar result.
...
Before proving the comparison principle, we begin by considering some properties of the intervention operator $\InterveneOper$ in Lemma \ref{lemma: Lemma 5.7 in Seydel}, as well as the perturbation technique presented in Lemma \ref{lemma: Lemma 5.10 in Seydel}.
...
\label{lemma: Lemma 5.10 in Seydel}
    Assume that conditions \assitemref{ass: V1} and \assitemref{ass: V2} are satisfied. Let $u$ be a subsolution and $v$ a supersolution of \eqref{eqn: QVI with functionals}. Suppose there exist a function $w \in \PB \cap C^{1,2}([0,T)\times \R^d)$ and a positive function $\kappa: [0,T] \times \R^d$ such that
    \begin{align*}
...
\end{lemma}
\begin{remark}
    Note that this is the parabolic extension to {\cite[Lemma 5.10]{seydel2010generalexistenceuniquenessviscosity}}.
\end{remark}
...
\end{quote}
In the proof of Theorem \ref{trm: 5.14 in Seydel}, $x_\varepsilon$ and $y_\varepsilon$ converge to the same limit $x_0$, so the requirement can be
relaxed to locally Lipschitz, as required by Assumption \ref{ass: Assumption 2.4 in Seydel}, and the property holds only for $x$ and $y$ in a suitable neighbourhood of $x_0$. 

Finally, recall that $\PB_m:=\PB_m ([0,T] \times \R^d)$ denotes the set of all functions $u \in \PB$ for which there is a time-dependent constant $C_t$ such that
\begin{equation*}
    |u(t,x)| \leq C_t (1+|x|^m) \quad \text{for all } (t,x) \in [0,T]\times \R^d.
\end{equation*}
The comparsion principle is presented in Theorem \ref{trm: 5.14 in Seydel}. 
...
\begin{proof}
    ...
    With the above assumptions in mind, we may apply Corollary \ref{cor: Corollary 5.13 in Seydel} at $(t_\varepsilon,x_\varepsilon,y_\varepsilon)$ with $\varphi(x,y) = \frac{1}{2\varepsilon} |x-y|^2$. Then, for any $\delta > 0$, there is a range of $\alpha>0$ for which there are matrices $X$ and $Y$ satisfying \eqref{eqn: (48) in Seydel}, $(p,-q) = \nabla \varphi(x_\varepsilon,y_\varepsilon)$, so that $p=q=\frac{1}{\varepsilon}(x_\varepsilon - y_\varepsilon)$, and $a-b= \partial_t \varphi(x_0,y_0) = 0$ such that
    \begin{equation*}
        \begin{aligned}
...
\begin{proof}
    ...
    \end{equation*}
    which shows that there exists at most one viscosity solution of \eqref{eqn: QVI with functionals}.
\end{proof}

Together, from the existence result in Chapter \ref{section: Viscosity Solution for Impulse Control: Existence} and the uniqueness result in the form of Corollary \ref{cor: Corollary 5.15 in Seydel}, we find that QVI \eqref{eqn: QVI with functionals} admits a unique viscosity solution under mild assumptions on the underlying functions and polynomial growth conditions. Moreover, since the value function \eqref{eqn: impulse control problem} satisfies the QVI in the viscosity sense, it follows that the value function is uniquely determined by the QVI. 

Although this gives a complete theoretical characterization, it generally does not yield a closed form representation. As is often the case with HJBQVIs, a closed-form solution may not exist and often depends on the choice of functions $f$, $g$, and $K$, as well as on the intervention operator $\InterveneOper$. This motivates the numerical schemes considered in the chapters to follow.