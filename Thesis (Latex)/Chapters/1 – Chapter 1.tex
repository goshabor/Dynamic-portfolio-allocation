...
An impulse control, modeled through a double-sequence $\alpha=\{(\tau_j,\zeta_j)\}_{j \geq 1}$, controls the jump diffusion process at intervention times $\tau_j$ through an impulse $\zeta_j$. If, for a fixed impulse control $\alpha$, the performance of an impulse control is quantified with the performance criterion $J^{(\alpha)}$, the impulse control problem translates into the maximization problem
\begin{equation*}
...
\begin{itemize}
    ...
    \item Chapter \ref{section: Numerical Experiment I: Impulse Control of the Foreign Exchange Rate} approximates the FEX rate problem using a finite-difference scheme and compares the scheme against DRL algorithms.
    \item Chapter \ref{section: Multi-dimensional Dynamic Portfolio Allocation} studies the use of impulse controls in dynamic portfolio allocation and uses the Proximal Policy Optimisation algorithm to approximate a trading policy in a portfolio consisting of 20 risky assets and one bank account. 
...
The theoretical foundations of this thesis were developed partly through coursework at École Polytechnique Fédérale de Lausanne (EPFL) and partly through self-study. In addition, several courses taken at NTNU provided useful background and intuition for the numerical and financial methods used throughout the thesis.

\noindent \textbf{Relevant courses taken at NTNU:}
\begin{itemize}
    \item \textit{FIN3009 – Asset Pricing and Portfolio Management}: Provided basic intuition for portfolio management and asset allocation.
    \item \textit{TMA4220 – Numerical Solution of Partial Differential Equations Using Element Methods}: Provided background on numerical methods for partial differential equations, including finite-difference schemes used for solving QVIs.
    \item \textit{TMA4215 – Numerical Mathematics}: Provided background on numerical algorithms, including the Fast Fourier Transform, which was used in the calibration procedure.
    \item \textit{TMA4500 – Specialisation project}: Provided experience with the use of the Fast Fourier Transform in the context of European option pricing.
\end{itemize}

\noindent \textbf{Relevant courses taken at EPFL:}
\begin{itemize}
    \item \textit{FIN417 – Quantitative Risk Management}: Provided background on risk management and risk measures for portfolio allocation, which was relevant for the dynamic portfolio allocation example.
    \item \textit{MATH450 – Numerical Integration of Stochastic Differential Equations}: Provided a deeper understanding of stochastic calculus and numerical schemes for stochastic differential equations.
    \item \textit{MATH470 – Martingales in Financial Mathematics}: Provided background in stochastic analysis, portfolio hedging, Lévy processes, and reinforcement-learning-based hedging methods, including Deep Hedging.
    \item \textit{MATH524 – Nonparameteric estimation and inference}: Provided rigorous results and proofs related to universal approximation theorems and statistical consistency results for neural networks.
\end{itemize}