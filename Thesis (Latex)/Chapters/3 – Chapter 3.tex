...
\label{def: Post-Jump, Pre-Intervention State}
    Let $\alpha$ be the impulse control from Definition \ref{def: Impulse Control}. We denote by $\check{X}^{(\alpha)}$ the state immediately after any exogenous Poisson jump at time $\tau_j$, but before the possible impulse at $\tau_j$, and define it as
    \begin{equation*}
...
\label{def: Post-intervention Function}
    Let $(t,x) \in [0,T] \times \Solvency$ be fixed, $\Solvency \subset \R^d$ be the state space and $\mathcal Z$ the set of admissible impulses. The post-intervention function
    \begin{equation*}
...
Let $\alpha=\{\tau_j,\zeta_j\}_{j \geq 1}^M$, $m\leq \infty$, be the impulse control from Definition \ref{def: Impulse Control}. Each of the impulse values $\zeta$ satisfies $\zeta \in \mathcal Z$, where $\mathcal Z$ denotes the non-empty set of admissible impulse values. Suppose now that at any time $t$, we can control the stochastic process $X$ using the control $\alpha$, and denote by $\Xa$ the controlled process.  Furthermore, suppose that applying an impulse $\zeta$ to the state $X(\tau^-)$ immediately moves the state from $\Xa(\tau^-)$ to $\Gamma(\tau,\check{X}^{(\alpha)}(\tau^-),\zeta) \in \Solvency$. Then, for some initial time $0 \leq t \leq T$, we define the controlled process $\Xa$ by
\begin{align}
...
\section{Impulse control via dynamic programming}
...
\begin{equation}
    v_n(t,x) = \sup_{\alpha_n \in \AdmissibleSet_n} J^{(\alpha_n)}(t,x), \quad n=1,2, \ldots \;.
...
\end{equation}
where $\mathcal{T}_t$ denotes the set of stopping times $\tau$ satisfying $\tau \leq \tauS \wedge T$ and $j$ denotes the remaining number of interventions. Equations \eqref{eqn: (10.1.11) in ØS} and \eqref{eqn: (10.1.12) in ØS} are referred to as the iterated optimal stopping representation. 

Our objective now is to show that the iterated optimal stopping representation is an approximation of the truncated impulse control problem \eqref{eqn: value function for at most n interventions}. This result takes shape in Theorem \ref{trm: Theorem 10.2 in ØS}, where Definition \ref{def: Space of Polynomially Bounded Functions} is used.

\begin{definition}[Space of Polynomially Bounded Functions] 
...
\end{definition}
\begin{theorem}[{\cite[Theorem 10.2]{oksendal2007applied}} Iterated optimal stopping representation]
...
Theorem \ref{trm: Theorem 10.2 in ØS} formulates the truncated impulse control problem into a sequence of standard optimal stopping problems. In particular, when the controller is restricted to at most $n$ interventions, the value function can be computed by backward recursion. After each intervention, the controller optimally selects when to intervene next and takes the solution from the previous step as the continuation value. This representation lets us replace a single, globally coupled impulse optimisation with an iterated sequence of simpler timing problems. However, to prove Theorem \ref{trm: Theorem 10.2 in ØS}, we need to consider a few auxiliary results about the Dynamic Programming Principle, which are stated in Lemma \ref{lemma: DPP} and Corollary \ref{cor: Corollary 10.5 in ØS}. 

\begin{lemma}[{\cite[Lemma 10.3]{oksendal2007applied}}, Dynamic Programming Principle]
\label{lemma: DPP}
    Let $X$ denote an uncontrolled jump-diffusion process, with $X(t^-)=x$. Suppose $G \in \PB([0,T)\times \R^d)$ and define 
    \begin{equation}
...
\end{lemma}
\begin{proof}
    As the proof of Lemma \ref{lemma: DPP} is based on theoretical results that are beyond the scope of this thesis, we refer the reader to \cite[Section 4]{ishikawa2004optimal} for a complete proof.
\end{proof}

\begin{corollary}[{\cite[Corollary 10.5]{oksendal2007applied}}]
\label{cor: Corollary 10.5 in ØS}
    Let $X$ denote an uncontrolled jump-diffusion process, with $X(t^-)=x$. 
    \begin{enumerate}[label=(\alph*)]
...
\begin{proof}[Proof of Theorem \ref{trm: Theorem 10.2 in ØS}]
    ...
    \end{equation*}
    Combined with \eqref{eqn: (10.1.28) in ØS}, the above result proves Theorem \ref{trm: Theorem 10.2 in ØS}.
\end{proof}
...
It remains to connect the truncated impulse control problem with the unrestricted impulse control problem, which is done through the following two corollaries. Corollary \ref{cor: Corollary 10.7 in ØS} combines Lemma \ref{lemma: Lemma 10.1 in ØS} with Theorem \ref{trm: Theorem 10.2 in ØS} to show that the iterated optimal stopping functions \eqref{eqn: (10.1.11) in ØS}--\eqref{eqn: (10.1.12) in ØS} monotonically converge to the value function $v$. Finally, Corollary \ref{cor: Corollary 10.8 in ØS} translates the impulse control problem \eqref{eqn: optimal value function} into a non-linear optimal stopping problem.
...
We conclude this chapter by noting that, after time discretisation, the dynamic programming principle may be applied to Markov decision processes and, consequently, to reinforcement learning. This is done in Chapter \ref{section: Deep Reinforcement Learning for Impulse Control Problems}. As such, the results shown in this section represent the core idea behind much of this thesis.