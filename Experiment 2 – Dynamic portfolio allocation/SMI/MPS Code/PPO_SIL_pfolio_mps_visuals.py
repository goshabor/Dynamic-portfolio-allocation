...
def _numpy_1d(x: Any) -> np.ndarray:
...
def _safe_sharpe_from_returns(
    ...
    *,
    ...
    """Compute a finite-sample Sharpe ratio from simple returns.

    ``annualization_factor`` is the multiplier inside the square root.  For
    returns observed over a horizon of length ``T`` years, use ``1 / T``.  For
    step returns with step length ``dt`` years, use ``1 / dt``.
    """
    r = _numpy_1d(returns)
    r = r[np.isfinite(r)]
    if r.size < 2:
    ...
    if not np.isfinite(std) or std <= float(eps):
        return float("nan")
    return float(np.sqrt(float(annualization_factor)) * float(np.mean(excess)) / std)


def _safe_divide_np(numerator: np.ndarray, denominator: np.ndarray, eps: float = 1.0e-12) -> np.ndarray:
    numerator = np.asarray(numerator, dtype=float)
    denominator = np.asarray(denominator, dtype=float)
    out = np.zeros(np.broadcast_shapes(numerator.shape, denominator.shape), dtype=float)
    return np.divide(numerator, denominator, out=out, where=np.abs(denominator) > float(eps))
...
def evaluate_policy(
    ...
    terminal, disc_terminal, disc_profit, initial, trades, costs, abs_trades = [], [], [], [], [], [], []
    h_sum = None
    s_sum = None
    cash_sum = None
    left = n_envs
    ...
    initial_t = torch.cat(initial).clamp_min(1.0e-12)
    trades_t = torch.cat(trades)
    costs_t = torch.cat(costs)
    abs_trade_t = torch.cat(abs_trades)

    terminal_return_t = terminal_t / initial_t - 1.0
    discounted_return_t = disc_terminal_t / initial_t - 1.0
    risk_free_return = math.exp(float(env.cfg.r) * float(env.cfg.T)) - 1.0
    horizon_annualizer = 1.0 / max(float(env.cfg.T), 1.0e-12)
    ...
    return {
        ...
        "terminal_return_mean": float(terminal_return_t.mean()),
        "terminal_return_std": float(terminal_return_t.std(unbiased=False)),
        "discounted_return_mean": float(discounted_return_t.mean()),
        "discounted_return_std": float(discounted_return_t.std(unbiased=False)),
        "sharpe_ratio": _safe_sharpe_from_returns(terminal_return_t, risk_free_return=risk_free_return),
        "annualized_sharpe_ratio": _safe_sharpe_from_returns(
            terminal_return_t,
            risk_free_return=risk_free_return,
            annualization_factor=horizon_annualizer,
        ),
        "discounted_sharpe_ratio": _safe_sharpe_from_returns(discounted_return_t),
        "annualized_discounted_sharpe_ratio": _safe_sharpe_from_returns(
            discounted_return_t,
            annualization_factor=horizon_annualizer,
        ),
...
def evaluate_single_path(
    ...
    env: VarianceGammaPortfolioEnv,
    deterministic: bool = True,
    price_paths: Optional[torch.Tensor] = None,
    asset_names: Optional[Iterable[str]] = None,
) -> Dict[str, Any]:
    """Run one evaluation path and return all data needed for diagnostics.

    This function records only one path, so it is intentionally separate from
    ``rollout_batch`` to avoid increasing training memory usage.  The policy and
    environment still run on ``env.device``; tensors are copied to CPU only for
    the returned trace.
    """
    agent.eval()
    steps = int(env.cfg.n_steps)
    state = env.reset(1, price_paths=price_paths)

    if asset_names is None:
        names = [f"Asset {i + 1}" for i in range(env.d)]
    else:
        names = [str(x) for x in asset_names]
        if len(names) != env.d:
            raise ValueError(f"asset_names must have length d={env.d}; got {len(names)}.")
    ...
    def _record_state(st: Dict[str, torch.Tensor]) -> None:
        prices.append(st["s"][0].detach().cpu().numpy().astype(float))
        holdings.append(st["h"][0].detach().cpu().numpy().astype(float))
    ...
    for _t in range(steps):
        ...
        obs_list.append(obs[0].detach().cpu().numpy().astype(float))
        interventions.append(int(act["intervene"][0].detach().cpu()))
        decision_prob.append(float(act["prob_d"][0].detach().cpu()))
        raw_actions.append(act["raw_action"][0].detach().cpu().numpy().astype(float))
        trade_intents.append(act["trade_intent"][0].detach().cpu().numpy().astype(float))
        safe_trades.append(info["safe_trade"][0].detach().cpu().numpy().astype(float))
    ...
    holdings_np = np.asarray(holdings, dtype=float)
    cash_np = np.asarray(cash, dtype=float)
    wealth_np = np.asarray(wealth, dtype=float)
    discounted_wealth_np = np.asarray(discounted_wealth, dtype=float)
    asset_values = holdings_np * prices_np
    risky_value = asset_values.sum(axis=1)
    risky_weights = _safe_divide_np(asset_values, risky_value[:, None])
    portfolio_weights = _safe_divide_np(asset_values, wealth_np[:, None])
    cash_weight = _safe_divide_np(cash_np, wealth_np)

    trade_np = np.asarray(safe_trades, dtype=float)
    time_grid = np.linspace(0.0, float(env.cfg.T), steps + 1)
    trade_time = time_grid[:-1]
    dt = float(env.dt)
    gross_step_returns = _safe_divide_np(wealth_np[1:], wealth_np[:-1]) - 1.0
    discounted_step_returns = _safe_divide_np(discounted_wealth_np[1:], discounted_wealth_np[:-1]) - 1.0
    risk_free_step_return = math.exp(float(env.cfg.r) * dt) - 1.0

    metrics = {
        "initial_wealth": float(wealth_np[0]),
        "terminal_wealth": float(wealth_np[-1]),
        "discounted_terminal_wealth": float(discounted_wealth_np[-1]),
        "terminal_return": float(wealth_np[-1] / max(wealth_np[0], 1.0e-12) - 1.0),
        "discounted_terminal_return": float(discounted_wealth_np[-1] / max(wealth_np[0], 1.0e-12) - 1.0),
        "total_cost": float(np.sum(costs)),
        "trade_count": int(np.sum(np.abs(trade_np).sum(axis=1) > 1.0e-8)),
        "mean_abs_trade": float(np.mean(np.abs(trade_np))) if trade_np.size else 0.0,
        "realized_sharpe_ratio": _safe_sharpe_from_returns(
            gross_step_returns,
            risk_free_return=risk_free_step_return,
            annualization_factor=1.0 / max(dt, 1.0e-12),
        ),
        "realized_discounted_sharpe_ratio": _safe_sharpe_from_returns(
    ...
    return {
        ...
        "prices": prices_np,
        "holdings": holdings_np,
        "cash": cash_np,
        "wealth": wealth_np,
        "discounted_wealth": discounted_wealth_np,
        ...
        "decision_prob": np.asarray(decision_prob, dtype=float),
        "raw_action": np.asarray(raw_actions, dtype=float),
        "trade_intent": np.asarray(trade_intents, dtype=float),
        "safe_trade": trade_np,
        ...
        "step_returns": gross_step_returns,
...
def plot_asset_allocation(
    ...
    *,
    ...
    title: str = "PPO-SIL risky allocation",
    ax: Any = None,
) -> Any:
    """Stacked-area plot of one evaluation path's allocation weights."""
    import matplotlib.pyplot as plt
    from matplotlib.ticker import PercentFormatter

    names = [str(x) for x in (asset_names if asset_names is not None else trace.get("asset_names", []))]
    time = np.asarray(trace["time"], dtype=float)
    weights = np.asarray(trace["risky_weights"], dtype=float)
    if not names:
        names = [f"Asset {i + 1}" for i in range(weights.shape[1])]
    if include_cash:
        weights = np.column_stack([weights, np.asarray(trace["cash_weight"], dtype=float)])
        names = names + ["Cash"]

    if ax is None:
        _, ax = plt.subplots(figsize=(12, 6))
    ax.stackplot(time, weights.T, labels=names)
    ax.set_title(title)
    ax.set_xlabel("Time")
...
def plot_impulses(
    ...
    *,
    asset_names: Optional[Iterable[str]] = None,
    title: str = "PPO-SIL impulses through time",
    trade_tolerance: float = 1.0e-8,
    ax: Any = None,
) -> Any:
    """Plot signed executed trades, in shares, for each asset over one path."""
    import matplotlib.pyplot as plt

    trades = np.asarray(trace["safe_trade"], dtype=float)
    time = np.asarray(trace["trade_time"], dtype=float)
    names = [str(x) for x in (asset_names if asset_names is not None else trace.get("asset_names", []))]
    if not names:
        names = [f"Asset {i + 1}" for i in range(trades.shape[1])]

    if ax is None:
        _, ax = plt.subplots(figsize=(12, 4.5))
    for j, name in enumerate(names):
        y = trades[:, j]
        nz = np.abs(y) > float(trade_tolerance)
        if nz.any():
            ax.plot(time[nz], y[nz], marker="o", linestyle="None", markersize=3.0, label=name)
        else:
            ax.plot([], [], marker="o", linestyle="None", markersize=3.0, label=name)
    ax.axhline(0.0, linewidth=1.0)
    ax.set_title(title)
    ax.set_xlabel("Time")
    ax.set_ylabel("Executed impulse / trade in shares")
    ax.grid(True, axis="y", alpha=0.25)
    ax.legend(loc="center left", bbox_to_anchor=(1.02, 0.5), fontsize=8)
    if not (np.abs(trades) > float(trade_tolerance)).any():
        ax.text(0.5, 0.5, "No impulses fired", transform=ax.transAxes, ha="center", va="center")
    return ax.figure
...
def plot_portfolio_value(
    trace: Dict[str, Any],
    *,
    title: str = "PPO-SIL portfolio value",
    include_discounted: bool = True,
    ax: Any = None,
) -> Any:
    """Plot raw and optionally discounted portfolio value for one path."""
    import matplotlib.pyplot as plt

    time = np.asarray(trace["time"], dtype=float)
    wealth = np.asarray(trace["wealth"], dtype=float)
    disc = np.asarray(trace["discounted_wealth"], dtype=float)
    if ax is None:
        _, ax = plt.subplots(figsize=(12, 4.5))
    ax.plot(time, wealth, label="Portfolio value")
    if include_discounted:
        ax.plot(time, disc, label="Discounted portfolio value")
    ax.set_title(title)
    ax.set_xlabel("Time")
    ax.set_ylabel("Value")
    ax.grid(True, axis="y", alpha=0.25)
...
def plot_single_path_diagnostics(
    trace: Dict[str, Any],
    *,
    asset_names: Optional[Iterable[str]] = None,
    ...
    """Create the requested one-path evaluation visualisations.

    Returns a dictionary with matplotlib figures for: ``asset_allocation``,
    ``impulses`` and ``portfolio_value``.
    """
    import matplotlib.pyplot as plt
    ...
    figs = {
        "asset_allocation": plot_asset_allocation(
            ...
            include_cash=include_cash_in_allocation,
        ),
        "impulses": plot_impulses(trace, asset_names=asset_names),
        "portfolio_value": plot_portfolio_value(trace),
    }
...
def evaluate_and_plot_single_path(
    ...
    *,
    ...
    """Convenience wrapper: run one path, compute metrics and plot diagnostics."""
    trace = evaluate_single_path(agent, env, deterministic=deterministic, asset_names=asset_names)
    figs = plot_single_path_diagnostics(
        trace,
        asset_names=asset_names,
        include_cash_in_allocation=include_cash_in_allocation,
        save_dir=save_dir,
        show=show,
    )
    return {"trace": trace, "figures": figs, "metrics": trace["metrics"]}
...
if __name__ == "__main__":
    ...
    cfg = PPOSILConfig(device="cpu", train_iters=2, rollout_envs=16, eval_envs=16, eval_batch_size=16,
                       ...
                       sil_warmup_iters=1, log_every=1, eval_every=1, checkpoint_path="/tmp/PPO_SIL_pfolio_mps_smoke.pt")
