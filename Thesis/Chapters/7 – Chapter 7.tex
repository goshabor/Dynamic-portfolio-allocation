...
\begin{remark}[DRL as an approximation of the viscosity solution]
    ...
    This interpretation is motivated by universal approximation theorems such as Theorem \ref{trm: Universal approximation theorem II}, as well as by the statistical consistency of neural networks. However, this should be understood as a heuristic approximation argument rather than a convergence proof. For this argument to hold, monotonicity, stability, and consistency results for the DRL schemes are needed. These results are not provided in this thesis and, to the best of our knowledge, are yet to be derived. A starting point for such a venture can be found in \cite{bayraktar2023approximate}. 
\end{remark}


% \subsection{Classifications of DRL models}
% %Explain offline and online models, explain off- and on-policies, explain value-based and policy/actor-critic-based approximations
% There exist several ways to group DRL models depending on how the agent is trained and what part of the RL problem is represented by a neural network. A first useful distinction is between \emph{online} and \emph{offline} RL. In offline RL, the agent does not interact with the environment while learning. Instead it is trained using a fixed dataset of previously collected transitions or paths. While offline models are important in application where trying new actions can be expensive or risky, for example in healthcare or robotics, they also require that the available dataset is sufficiently informative about good decisions. In particular, since the agent cannot explore the environment on its own, its performance is limited by the quality and coverage of the data it is trained on.  

% By contrast, in online RL, the agent learns by directly interacting with environment. By repeatedly observing the current state, selecting an action, receiving a rewards, and then using this newly generated experience, it improves its future behaviour to make optimal decisions. This setting is especially useful in simulated environments, where new states or paths can be generated at relatively low cost and the agent can gradually improve through trial and error. 

% A common example of online DRL algorithms are policy-gradient methods. In these methods, the policy itself is parametrised by a neural network, that is $\pi_\theta(x|a) \approx \pi(x|a)$, and the parameters $\theta$ are updated using the gradient of the policy with respect to $\theta$. Thus, rather than first learning a value function and then deriving a policy from it, policy-gradient methods optimise the agent’s behaviour directly. This makes them particularly useful when the action space is continuous or when one wants to model stochastic decision-making.

% Another important distinction is between \emph{on}- and \emph{off}-policy methods. An on-policy method learns from data generated by the same policy that is currently being updated, i.e. after each policy update, new trajectories are typically collected using the updated policy before further training is performed. Many policy-gradient methods, including the PPO algorithm which will be introduced later in this chapter, are commonly viewed as on-policy methods. On the other hand, off-policy methods are more flexible in the sense that they can learn from data generated by a different policy rather than the one currently being improved. This makes it possible to reuse old experiences, often stored in a replay buffer $\mathcal{B}_\text{replay}$, and can make learning more sample-efficient. Standard examples of an off-policy include the deep Q-learning network (DQN) and the double deep-Q-learning network (DDQN). 

% With online and offline algorithms, as well as on- and off-policies in mind, we note that the key point in these models is that online and offline models describe whether the agent can collect new data during training, whereas on- and off-policies describe whether the training data must come from the current policy or not. 

% Finally, DRL methods are also classified according to which methematical object is being approximated by a neural network. In \emph{value-based} methods, the neural network approximates the value function. For instance, in DQN models, the function $Q$ is approximated by the neural network and the value function is then computed by computing the expectation of the Q-functions over different states and actions via the Bellman equations \eqref{eqn: Bellman equations}. The policy is then obtained indirectly by choosing actions that give highest returns at each time step\footnote{Such an approach is often referred to as a greedy method.}. A common example of such methods are the DQN and the DDQN algorithms. In \emph{policy-based} methods, the policy itself is modeled directly as a parametrised function $\pi_\theta$, and the parameters are adjusted so as to increase expected future rewards for example via policy gradients. A third and very widely used class is given by \emph{actor-critic} methods. Actor-critic methods combine the two previous ideas so that the actor represents the policy and decided which actions to take, while the critic estimates a value function and evaluates how good those actions are. That way, actor-critic methods use value-function learning to guide direct policy optimisation. Many modern DRL algorithms, including PPO, fall into this category.

% In the next parts of this chapter, we will relate the impulse control problem to value-based and to actor-critic methods. Specifically, in Chapter \ref{section: Value-based Approximation of the Impulse-Control Problem}, we will discuss the use of the DDQN algorithm to approximate the value function $v$ resulting from the impulse control problem, as well as the use of the PPO algorithm to both learn the optimal policy, i.e. when and how to intervene, and the value function. 
...
\begin{equation}
    Q_\theta(t_n,x,0) = \E^{(t_n,x)}\left[\int_{t_n}^{t_{n+1}} f(s,X(s)) \diff s + v(t_{n+1}, X(t_{n+1})) \right]
    \label{eqn: DDQN Q-func. temp 1}
\end{equation}
Using a left-point approximation of the integral, \eqref{eqn: DDQN Q-func. temp 1} becomes
\begin{equation}
    Q_\theta(t_n,x,0) \approx f(t_n,x) \Delta t + \E^{(t_n,x)}\left[V(t_{n+1}, X(t_{n+1})) \right]. 
    \label{eqn: Q-value no intervention}
\end{equation}
For an action $a_n \neq 0$, the Q-value is given by
\begin{equation}
    Q(t_n,x,a_n) = K(t_n,x,a_n) + V(t_n, \Gamma(t_n,x,a_n)),
...
\begin{equation*}
    Q_\theta(t_n,x,a) \approx r(t_n,x,a_n) + \E^{(t_n,x,a_n)}\left[v(t_{n+1}, X^{(\beta)}(t_{n+1})) \right],
\end{equation*}
...
A prominent actor–critic algorithm that commonly employs a shared network is the Proximal Policy Optimisation (PPO) algorithm, which we further augment with Self‑imitation Learning (SIL) and entropy regularisation. As discussed later in this chapter, these augmentations provide additional stability and encourage effective exploration. The combination of PPO with SIL is inspired by the work of \textcite{oh2018self}, where it is shown to improve policy learning. Furthermore, the PPO-SIL approach was first applied on impulse control problems in finance by \textcite{jain2025impulse}, where it was empirically shown to be effective in market making applications.
...
\begin{equation*}
    \zeta \sim \pi_\xi \left( \cdot | x \right). 
\end{equation*}
If $d = 0$, no impulse is applied. Using the critic $V_{\chi, \xi}$, which is the approximation of the policy-value function under the current joint policy $\pi_{\chi, \xi}$ (see Definition \ref{def: Policy-value function}), the actor network approximates
\begin{equation*}
...
\begin{equation}
    \pi_{\chi, \xi}(a | x) = 
    \begin{cases}
        \pi_\chi \big(0| x\big), & \quad \text{when } d = 0, \\[1ex]
        \pi_\chi \big(1|x \big) \pi_\xi(\zeta | x), & \quad \text{when } d = 1.
    \end{cases}
    \label{eqn: PPO joint policy}
\end{equation}
When it comes to the neural network parameter updates, we note that the impulse-size network $u_\xi$ is updated using only time steps at which the switch $d$ was active (i.e. when intervention was made). In contrast, the decision network $d_\chi$ is updated at every time step because every time step requires a decision between continuation and intervention. 
...
\end{align}
where $\hat G_n$ is given by \eqref{eqn: GAE}.
...
\end{equation}
Finally, for a fixed time $t$ and state $x$, the entropy \eqref{eqn: entropy regularisation} is derived via
\begin{align}
...
\end{align}
The entropies $\mathcal{H}(\pi_\chi)$ and $\mathcal{H}(\pi_\xi)$ in \eqref{eqn: L-entropy with 2 heads} are computed using \eqref{eqn: entropy regularisation}.
...
\subsection{Implementation and training procedure}
As in the DDQN model discussed in Chapter \ref{section: Value-based Approximation of the Impulse-Control Problem}, we use two feedforward neural networks with identical architecture and initial weights $\chi_\text{init}$ and $\xi_\text{init}$, respectively. However, the main difference is now that one network outputs the decision $d_\chi \in \{0,1\}$ and the other network outputs the impulse-size $u_\xi \approx \zeta$. Both networks use the hyperbolic tangent \eqref{eqn: Tanh activation} as activation function. For the implementation, the code written by \textcite{jain2025impulse} is used, see \cite{jain2025impulsecode}.

The PPO-SIL algorithm is trained using simulated rollouts\footnote{A rollout is one simulation of the controlled process.}. Each rollout produces a full path of states, actions, rewards, and pointwise value function estimates. Then, at the beginning of each training iteration, a sample of paths of the uncontrolled process $X$ is generated. Conditional on these paths, the current joint policy $\pi_{\chi, \xi}$, which is given by \eqref{eqn: PPO joint policy}, is applied step by step along the time grid. That is, at each time step $t_n$, the policy observes the current controlled state $X_n^{(\beta)}$ and samples an action $a_n$. If intervention is chosen, the state is moved from $X_n^{(\beta)}$ to $\Gamma(t_n,\check{X}_n^{(\beta)},\zeta_n)$. Otherwise, the process evolves without intervention until the next decision time. This approach results in a pathwise realisation of the controlled process, and the corresponding rewards, actions, value estimates, and log-probabilities are collected and stored inside the SIL buffer.

Let the superscript $m$ denote the $m$-th rollout path of the controlled process $X^{(\beta)}$ and let $\beta^m=(a_0^m, a_1^m, \ldots, a_{N_t}^m)$ denote the corresponding strategy. Return-to-go \eqref{eqn: return-to-go} and generalised advantage estimate \eqref{eqn: GAE} are computed for each simulated path. The PPO loss is then approximated using
\begin{align}
