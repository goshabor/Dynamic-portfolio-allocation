...
\end{equation}
For a stopping time $\tau':=\tau_\Solvency \wedge T$, where $T<\infty$ the time horizon, the performance criterion $J$ is given by
\begin{equation*}
...
\end{equation*}
The functions $f$, $g$, and $K$ are assumed to satisfy the integrability conditions in \eqref{eqn: well-behavedness of f}–\eqref{eqn: well-behavedness of K}. These conditions ensure that the performance criterion is finite for every admissible control $\alpha \in \AdmissibleSet$ and every $(t,x) \in [0,T] \times \Solvency$. The uncontrolled process $X$ follows the SDE \eqref{eqn: jump-diffusion SDE}, with drift, diffusion, and jump-amplitude functions satisfying \eqref{eqn: Lipschitz condition}--\eqref{eqn: lgb condition}, and the controlled process $\Xa$ is given by \eqref{eqn: (9.1.2) in ØS}--\eqref{eqn: (9.1.4) in ØS}. Furthermore, recall that the intervention operator $\InterveneOper$, introduced in Definition \ref{def: Intervention Operator}, is given by
\begin{equation*}
    \InterveneOper v(t,x) = \sup_{\zeta \in \mathcal{Z}} \big\{ v\big(t,\Gamma(t,x,\zeta)\big) + K(t,x,\zeta) \big\}
\end{equation*}
...
\label{ex: solvency time notice}
    ...
    Now suppose that $X^{(\alpha)}$ models the cash account of a profitable business. Let $\Solvency = (0,\infty)$ and $X^{(\alpha)}(\tau^-) = 1 \in \Solvency$ be such that the firm begins in a solvent state. If a negative size shock $-4$ occurs at time $\tau$, the post-jump pre-intervention state is $\check{X}^{(\alpha)}(\tau^-) = 1-4 = -3 \notin \Solvency$ and the profitable business has run out of capital. To push the state back into the solvency region, investors may inject capital into the firm through an impulse, say $\zeta=5$. Then the state jumps back into solvency, i.e. $\Gamma(\tau, \check{X}^{(\alpha)},\zeta) = -3 + 5 = 2 \in \Solvency$. Thus, the business becomes solvent again.
\end{example}
...
As discussed in Chapter \ref{section: Preliminaries}, we assume that the basic regularity conditions \eqref{eqn: Lévy measure condition} and \eqref{eqn: assumption for chapter 4} are satisfied. Then, with \eqref{eqn: assumption for chapter 4} and according to \cite{seydel2010generalexistenceuniquenessviscosity}, the Hamiltonian $\partial_t u + \InfinitesimalGenerator u + f$ is well-defined if, for instance, the following conditions are satisfied
\begin{enumerate}
...
\label{def: Definition 4.1 in Seydel (Viscosity solution)}
    Denote $\PB = \PB([0,T]\times \R^d)$. A function $u \in \PB$ is a viscosity subsolution of \eqref{eqn: QVI} if for all $(t_0,x_0) \in [0,T] \times \R^d$ and $\varphi \in \PB \cap C^{1,2}([0,T]\times \R^d)$ with $\varphi(t_0,x_0) = u^*(t_0,x_0)$, where $u^*$ is the upper semi-continuous envelope of $u$ (see Definition \ref{def: upper semi-continuous functions}), and with $\varphi \geq u^*$ on $[0,T] \times \R^d$, we have
    \begin{align*}
...
\begin{proof}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \begin{align*}
            ...
            & \InterveneOper u_* (t,x) - \varepsilon.
        \end{align*}
...
\begin{proof}
    ...
    With this in mind, we now aim to show that $\bar \tau_n \rightarrow t_0$ in probability. Therefore, we begin by observing that, for some $t,x) \in [0,T] \times \Solvency$, $\tau = \tauS \wedge T$, and $\tilde \tau \leq \tau$, the performance criterion satisfies the following inequality
    \begin{align}
