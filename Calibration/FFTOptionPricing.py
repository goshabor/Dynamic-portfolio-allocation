...
class OptionPricingFFT:
    def __init__(self, S0, r, mu_Q, T, VG_params, CarrMadan_params):
        self.S0 = S0
        self.r = r
        self.mu_Q = mu_Q #=r-q
        self.T = T
...
class Calibration:
    def __init__(self, df, S0, r, VG_params_init, CarrMadan_params):
        ...
        relevant_columns = ['price', 'vega', 'drift']
        ...
        for i, T in enumerate(maturities):
            ...
            cols_needed = [0, 1, 2, 3]  # strike, price, vega, drift
            ...
            vega_i = data_clean[:,2]
            #drift_i = self.r
            drift_i = data_clean[:,3]

            mu_Q = float(np.median(drift_i))
            pricer = OptionPricingFFT(S0=self.S0, r=self.r, mu_Q=mu_Q, T=T, VG_params=self.VG_params_init, CarrMadan_params=self.CarrMadan_params)
    ...
    def objective_func(self, VG_params):
        ...
        for entry in self.per_T:
            ...
            # ok = np.isfinite(C_interpolated) & np.isfinite(close) & np.isfinite(vega) & (close > 0) & (vega > 1e-8)
            # if not np.any(ok):
            #     continue
            
            # rel_error = (C_interpolated[ok]-close[ok])/(close[ok])
            rel_error = (C_interpolated-close)/close
            total_error += np.sum(rel_error**2) # + nu
